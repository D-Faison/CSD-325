...
def main():
    ...
    #display Lake with the forest as parameters
    placeLake(forest)
    bext.clear()
...
#Lake function with forest parameter
def placeLake(forest):
    #size of lake
    lake_width = 15
    lake_height = 5

    #width and height of constraints  minus the height and width of the lake
    #divided by two, double // so its rounded to the nearest whole number

    start_x = (WIDTH - lake_width)//2
    start_y = (HEIGHT - lake_height)//2

    #for each increment in range from 15 to 47 horizantally 
    for x in range(start_x,start_x + lake_width):
        # for each increment in range from 8 to 13 vertically
    ...
    for y in range(forest['height']):
        for x in range(forest['width']):
            if forest[(x, y)] == TREE:
                ...
                bext.fg('red')
                print(FIRE, end='')
            #while iterating through the forest, if it is lake, display it as blue
